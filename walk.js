...
function changeDirectionIfNecessary(ball, x, y) {
  if (x < 0 || x > area.width - ball.width) {
    //xaxis is beyond frame so changing the xaxis value to negative
    ball.dx = -ball.dx;
  }
  if (y < 0 || y > area.height - ball.height) {
    //yaxis is beyond frame so changing the yaxis value to negative
    ball.dy = -ball.dy;
  }
...
// newBall.element.style.left=dx;
// newBall.element.style.top=dy;
...
function update(ball, x, y)
 {
  ...
  //It flips the ball by adding the changed direction width and height
